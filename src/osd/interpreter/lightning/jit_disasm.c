 ...
 */
#if DISASSEMBLER
static int disasm_compare_symbols(const void *ap, const void *bp);

static void disasm_print_address(bfd_vma addr, struct disassemble_info *info);

#define disassemble(u, v) _disassemble(_jit, u, v)
static void _disassemble(jit_state_t *_jit, jit_pointer_t code, jit_int32_t length);
#endif
 ...
 * Implementation
 */
void jit_init_debug(const char *progname)
{
#if DISASSEMBLER
    bfd_init();
...
#if BINUTILS_2_29
    disasm_print = disassembler(disasm_info.arch, __BYTE_ORDER == __BIG_ENDIAN, disasm_info.mach, disasm_bfd);
#else
    ...
    if (bfd_get_file_flags(disasm_bfd) & HAS_SYMS)
    {
        ...
        if ((sym_storage = bfd_get_symtab_upper_bound(disasm_bfd)) >= 0)
        {
            ...
            jit_alloc((jit_pointer_t *)&disasm_symbols, (sym_storage + dyn_storage) * sizeof(asymbol *));
            sym_count = bfd_canonicalize_symtab(disasm_bfd, disasm_symbols);
            assert(sym_count >= 0);
            if (dyn_storage)
            {
                dyn_count = bfd_canonicalize_dynamic_symtab(disasm_bfd, disasm_symbols + sym_count);
                assert(dyn_count >= 0);
            }
            else
                dyn_count = 0;
            ...
            disasm_num_synthetic = bfd_get_synthetic_symtab(disasm_bfd, sym_count, disasm_symbols, dyn_count,
                                                            disasm_symbols + sym_count, &disasm_synthetic);
            if (disasm_num_synthetic > 0)
            {
                jit_realloc((jit_pointer_t *)&disasm_symbols, (sym_storage + dyn_storage) * sizeof(asymbol *),
                            (sym_storage + dyn_storage + disasm_num_synthetic) * sizeof(asymbol *));
                for (offset = 0; offset < disasm_num_synthetic; offset++)
                    disasm_symbols[disasm_num_symbols++] = disasm_synthetic + offset;
            }
            ...
            {
                symbol = *in++;
                if (symbol->name && symbol->name[0] != '\0' && !(symbol->flags & (BSF_DEBUGGING | BSF_SECTION_SYM)) &&
                    !bfd_is_und_section(symbol->section) && !bfd_is_com_section(symbol->section))
                    *out++ = symbol;
            }
            disasm_num_symbols = out - disasm_symbols;
            qsort(disasm_symbols, disasm_num_symbols, sizeof(asymbol *), disasm_compare_symbols);
        }
    }
#endif
}
...
#undef jit_pointer_p
#define jit_pointer_p(u) ((u) >= _jit->code.ptr && (u) < _jit->pc.uc)
    if (jit_pointer_p((jit_uint8_t *)(jit_word_t)addr))
    {
        if (jit_get_note((jit_uint8_t *)(jit_word_t)addr, &name, &file, &line))
            (*info->fprintf_func)(info->stream, " %s:%s:%d", name ? name : "", file ? file : "", line);
    }
#undef jit_pointer_p
#undef _jit
...
static void _disassemble(jit_state_t *_jit, jit_pointer_t code, jit_int32_t length)
{
    ...
    again:
        ...
        (*disasm_info.fprintf_func)(disasm_stream, "%*c0x%s\t", 16 - bytes, ' ', buffer);
