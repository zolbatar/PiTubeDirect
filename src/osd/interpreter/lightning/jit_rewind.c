 ...
 * Implementation
 */
static jit_node_t *_free_synth_list(jit_state_t *_jit, jit_node_t *node)
{
    jit_node_t *next;
    next = node->next;
    free_node(node);
...
static void _rewind_prolog(jit_state_t *_jit)
{
    jit_node_t *node;
    jit_node_t *next;
    ...
    _jitc->function->self.size += 64;
#endif
    _jitc->function->self.argi = _jitc->function->self.argf = _jitc->function->self.argn = 0;
...
static void _rewind_prepare(jit_state_t *_jit)
{
    jit_node_t *node;
    jit_node_t *next;
