...
jit_node_t *_jit_name(jit_state_t *_jit, const char *name)
{
    jit_node_t *node;
...
jit_node_t *_jit_note(jit_state_t *_jit, const char *name, int line)
{
    jit_node_t *node;
    ...
    if (_jitc->note.note == NULL || (name == NULL && _jitc->note.note != NULL) ||
...
void _jit_set_note(jit_state_t *_jit, jit_note_t *note, char *file, int lineno, jit_int32_t offset)
{
    ...
    if (note->length && index == note->length && note->lines[index - 1].file == file)
        --index;
    ...
    else
    {
        ...
        else
        {
            ...
            if ((line->length & 15) == 0)
            {
                jit_realloc((jit_pointer_t *)&line->linenos, line->length * sizeof(jit_int32_t),
                            (line->length + 17) * sizeof(jit_int32_t));
                jit_realloc((jit_pointer_t *)&line->offsets, line->length * sizeof(jit_int32_t),
                            (line->length + 17) * sizeof(jit_int32_t));
            }
...
jit_bool_t _jit_get_note(jit_state_t *_jit, jit_pointer_t code, char **name, char **file, jit_int32_t *lineno)
{
    jit_note_t *note;
    jit_line_t *line;
    jit_int32_t index;
    ...
    if ((jit_uint8_t *)code < note->code || (jit_uint8_t *)code >= note->code + note->size)
        return (0);
...
static jit_note_t *_new_note(jit_state_t *_jit, jit_uint8_t *code, char *name)
{
    jit_note_t *note;
    jit_note_t *prev;
...
static void new_line(jit_int32_t index, jit_note_t *note, char *file, jit_int32_t lineno, jit_int32_t offset)
{
    ...
    if (note->lines == NULL)
        ...
        jit_realloc((jit_pointer_t *)&note->lines, note->length * sizeof(jit_line_t),
                    (note->length + 17) * sizeof(jit_line_t));

    if (index < note->length)
        jit_memmove(note->lines + index + 1, note->lines + index, sizeof(jit_line_t) * (note->length - index));
    line = note->lines + index;
    ++note->length;
...
static jit_int32_t _note_search_index(jit_state_t *_jit, jit_uint8_t *code)
{
    ...
    for (index = (bot + top) >> 1; bot < top; index = (bot + top) >> 1)
    {
        if (code < notes[index].code)
            top = index;
        else if (code >= notes[index].code && code - notes[index].code < notes[index].size)
            break;
        else
            bot = index + 1;
    }
...
static jit_int32_t line_search_index(jit_note_t *note, jit_int32_t offset)
{
    ...
    for (index = (bot + top) >> 1; bot < top; index = (bot + top) >> 1)
    {
        ...
        else if (index == note->length - 1 || (offset >= *lines[index].offsets && offset < *lines[index + 1].offsets))
            break;
        else
            bot = index + 1;
    }
...
static jit_int32_t offset_search_index(jit_line_t *line, jit_int32_t offset)
{
    ...
    for (index = (bot + top) >> 1; bot < top; index = (bot + top) >> 1)
    {
        if (offset < offsets[index])
            top = index;
        /* offset should be already verified to be in range */
        else if (index == line->length - 1 || (offset >= offsets[index] && offset < offsets[index + 1]))
            break;
        else
            bot = index + 1;
    }
