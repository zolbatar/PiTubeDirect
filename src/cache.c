...
void InvalidateDataCache(void)
{
    ...
    for (nSet = 0; nSet < L1_DATA_CACHE_SETS; nSet++)
    {
        ...
        {
            nSetWayLevel = nWay << L1_SETWAY_WAY_SHIFT | nSet << L1_SETWAY_SET_SHIFT | 0 << SETWAY_LEVEL_SHIFT;
    ...
    for (nSet = 0; nSet < L2_CACHE_SETS; nSet++)
    {
        ...
        {
            nSetWayLevel = nWay << L2_SETWAY_WAY_SHIFT | nSet << L2_SETWAY_SET_SHIFT | 1 << SETWAY_LEVEL_SHIFT;
            asm volatile("mcr p15, 0, %0, c7, c6,  2" : : "r"(nSetWayLevel) : "memory"); // DCISW
        }
    }
}
...
void enable_MMU_and_IDCaches(void)
{
    ...
    for (base = 0; base < (L2_CACHED_MEM_BASE >> 20); base++)
    {
        ...
        // Values from RPI2 = 11C0E (outer and inner write back, write allocate, shareable (fast but unsafe)); works on
        // RPI Values from RPI2 = 10C0A (outer and inner write through, no write allocate, shareable) Values from RPI2 =
        // 15C0A (outer write back, write allocate, inner write through, no write allocate, shareable)
